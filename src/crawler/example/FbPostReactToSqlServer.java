 ...
 * 爬FB粉絲團每篇文章底下的按讚、LOVE、WOW....資訊並批次更新至資料庫
 */
...
//        loadFanClubId(); // 載入所有粉絲團ID
        ...
        try (Connection conn = DriverManager
                .getConnection(
                        ...
                        "sa", "sa123456");
             PreparedStatement pstmt = conn.prepareStatement(" insert into FbPostsReacts (postId, reactId, reactName, reactType)"
                     + "values (?, ?, ?, ?)")
        ) {
                ...
                try {
                    ...
                    String uri =
                            "https://graph.facebook.com/v2.8"
                                    ...
                                    + URLEncoder.encode("posts{created_time,reactions{id,name,type}}", "UTF-8")
                                    + "&access_token=" + token;
...
//
                                        ...
                                        if (checkPostId(postId)) {
                                            ...
                                            Elements eleReacts = postsLev2.get(1).children();
//                            System.out.println(eleComments.size()); // 看<comments>tag底下有幾個<data>tag
                                            Elements nextPageReact;

                                            while (true) {

                                                for (int j = 0; j < eleReacts.size() - 1; j++) {  // 一筆一筆取<comments>_<data>tag (最多顯示25篇)

                                                    tempId = eleReacts.get(j).getElementsByTag("id").text().split(" ");
                                                    reactId = tempId[0];
...
//                                                    insertToSqlServer(postReactInfo, pstmt);
                                                ...
                                                // Comment換頁
                                                nextPageReact = eleReacts.last().getElementsByTag("next");
                                                if (!nextPageReact.hasText())
                                                    break;
                                                else {
                                                    Document elems3 = CrawlerPack.start().getFromJson(nextPageReact.text().replaceAll("\\|", "%7C"));
                                                    eleReacts = elems3.children();
                                                }
